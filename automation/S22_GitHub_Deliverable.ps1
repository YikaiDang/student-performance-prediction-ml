...
if (-not (Test-Path -LiteralPath $PythonExe)) {
    ...
    if ($PythonLauncher[0] -eq 'py') {
        ...
        & $PythonLauncher[0] -m venv $VenvRoot
    }
...
& $PythonExe -c $DependencyCheck 2>$null
if ($LASTEXITCODE -ne 0) {
    Write-Host 'Installing Python packages...'
    & $PythonExe -m pip install --upgrade pip
    Assert-LastExitCode 'pip upgrade'

    & $PythonExe -m pip install pandas scikit-learn pytest
    Assert-LastExitCode 'Python package installation'
}
else {
    Write-Host 'Required Python packages are already available.'
}

Write-Section 'CREATING SESSION 22 FILES'
$SrcDir = Join-Path $ResolvedRepoRoot 'src'
$TestsDir = Join-Path $ResolvedRepoRoot 'tests'
...
It creates comparable train/test splits for both modelling scenarios.
'@

Set-Content -LiteralPath $PreprocessPath -Value $UtilityCode -Encoding utf8
Set-Content -LiteralPath $TestPath -Value $TestCode -Encoding utf8
Set-Content -LiteralPath $DocPath -Value $DocContent -Encoding utf8

Write-Host "Created: $PreprocessPath"
Write-Host "Created: $TestPath"
Write-Host "Created: $DocPath"

Write-Section 'VALIDATING PYTHON IMPLEMENTATION'
& $PythonExe -m py_compile $PreprocessPath
Assert-LastExitCode 'Python compilation'

Write-Host 'Python compilation passed.'

Write-Section 'RUNNING TESTS'
& $PythonExe -m pytest $TestPath -v
Assert-LastExitCode 'Session 22 tests'

Write-Host 'Session 22 tests passed.'